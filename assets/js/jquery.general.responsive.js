  ...
  */
  ...
  ResponsiveImage = (function() {
    ...
    function ResponsiveImage(node, changeEventName, restoreEventName) {
      ...
      if(this.changeContents[0] == "image"){
        this.baseContents = this.$node.attr('src');
      } else if(this.changeContents[0] == "link"){
        this.baseContents = this.$node.attr('href');
      }
    ...
    ResponsiveImage.prototype.setEvent = function() {
      ...
      events[this.changeEventName] = function() {
        if(_this.changeContents[0] == "image"){
          _this.$node.attr('src', _this.changeContents[1]);
        } else if(_this.changeContents[0] == "link"){
          _this.$node.attr('href', _this.changeContents[1]);
        }
        // _this.$node.remove();
      ...
      events[this.restoreEventName] = function() {
        if (_this.state > 0) {
          if(_this.changeContents[0] == "image"){
            _this.$node.attr('src', _this.baseContents);
          } else if(_this.changeContents[0] == "link"){
            _this.$node.attr('href', _this.baseContents);
          }
